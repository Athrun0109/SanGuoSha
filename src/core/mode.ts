...
export interface GameMode {
  ...
  /**
   * **圆桌上的实际座次**,按座位号列出。省略 = 座位号就是座次(绝大多数模式)。
   *
   * 存在的理由:三国杀把两件事绑在了同一个圈上 —— **谁下一个行动**(回合顺序、
   * 下家、濒死求桃顺序、AOE 结算顺序)和**谁离谁近**(距离、射程)。
   * 2v2 里这两件事要的排布正好冲突:
   *
   *   回合顺序想要「甲乙乙甲」—— 先手方走第一个和第四个,先手优势才抵消得掉
   *   距离想要「甲乙甲乙」—— 两个敌人都在身边,才谈得上集火
   *
   * 拆开就都能满足:数组顺序仍是甲乙乙甲(回合顺序不变、平衡不变),
   * 而圆桌另排成 0-1-3-2,于是每个人的两个邻座都是敌人、队友在距离 2。
   * 这样**用【杀】误伤队友也变成了不可能** —— 20260821-202633 那次事故的结构性根因。
   *
   * 只有 distance() 读这个字段;别处一律继续按数组顺序走,那正是回合顺序。
   */
  ring?(n: number): number[];
}
...
/**
 * **回合顺序** [蓝, 红, 红, 蓝]。注意这是顺序,不是圆桌座次 —— 座次见 `ring`。
 *
 * 引擎按数组下标推进回合,所以这么排之后出牌顺序就是「蓝→红→红→蓝」:
 * 先手方走第一个和第四个,中间两个连着给后手方,正好抵消先手优势
 * (实测 2000 局 48.9% / 51.1%;排成「甲乙甲乙」则是 58.6% / 41.4%)。
 *
 * 副作用是跨轮次时下标 3 和 0 都是蓝队,蓝队会连着走两个回合 —— 这是这种排法的固有结果。
 */
const TEAM_TABLE: Role[] = ['blue', 'red', 'red', 'blue'];

/**
 * 2v2 的阵营分配(决定**回合顺序**,不是圆桌座次)。
 *
 * 注意这里**不洗牌** —— 身份局里座位是随机的,而 2v2 的回合顺序本身就是规则的一部分:
...
function teamLayout(table: Role[], fixed?: Record<number, Role>): Role[] {
  ...
  throw new Error(
    `这份指定排不出 2v2 的阵营。下标顺序必须是「甲乙乙甲」(0、3 号位一队,1、2 号位一队)——
` +
    `这样出牌顺序才是 队1→队2→队2→队1,先手优势才抵消得掉。
` +
    `(圆桌上的实际座次是另一回事,由 ring 决定:每个人的两个邻座都是对手。)`);
}
...
export const team2v2Mode: GameMode = {
  ...
  /*
   * 圆桌另排:数组顺序(=回合顺序)是 蓝红红蓝,而实际座次是 0-1-3-2。
   * 于是 0/3 号位(蓝队)在圆桌上隔着一个人,两人的邻座都是红队 —— 反之亦然。
   * 见 GameMode.ring 的注释。
   */
  ring: () => [0, 1, 3, 2],
  // 没有主公,谁都不加血
