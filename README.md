...
| | 内容 |
|---|---|
| 牌堆 | **104 张**,基本牌 53 / 锦囊 34 / 装备 17 —— 花色点数逐格照抄官方牌表 |
| 基本牌 | 杀、闪、桃 |
| 锦囊 | 过河拆桥、顺手牵羊、决斗、无中生有、南蛮入侵、万箭齐发、桃园结义、五谷丰登、借刀杀人、无懈可击、乐不思蜀、闪电 |
| 装备 | 诸葛连弩、雌雄双股剑、青釭剑、青龙偃月刀、丈八蛇矛、贯石斧、方天画戟、麒麟弓、八卦阵×2、进攻马/防御马各三匹<br>(【仁王盾】是军争篇的牌,实现留着但不进标准版牌堆) |
| 武将 | 标准包 25 将全员(魏7 蜀7 吴8 群3),含主公技护驾/激将/救援 |
| 模式 | 2~8 人身份局(主公 +1 体力、击杀反贼摸三张、主公误杀忠臣弃牌);4 人 2v2 组队对抗 |
| 规则 | 完整的时机/响应链:无懈可击可连环、濒死依次求桃、判定可被鬼才替换、目标可被流离转移 |

规则实现对照过官方原文和 FAQ,逐条结果见 **[docs/rules-audit.md](docs/rules-audit.md)** ——
那里也记着**有意偏离官方**的几处(离间不可被无懈、桃园结义跳过满血、克己的 32 张房规上限…),
改规则之前先看一眼,免得把有意为之的东西"顺手修好"。对照用的官方语料在
[docs/faq/](docs/faq/):27 张牌的效果原文 + 142 条 FAQ,只收标准版。

## 2v2 组队对抗

4 人两队,**没有主公、队伍公开、没有击杀奖励** —— 把身份推理这一整块变量拿掉之后,
剩下的就只有配合本身。这是给 AI-AI 和人-AI 协同做实验用的模式。

**座次和出牌顺序是分开的** —— 三国杀原本把两件事绑在同一个圈上,而 2v2 里它们要的排布正好冲突:

| | 要什么 | 为什么 |
|---|---|---|
| 出牌顺序 | 甲→乙→乙→甲 | 先手方走第一个和第四个,先手优势才抵消得掉(否则实测 58.6% vs 41.4%) |
| 圆桌座次 | 甲乙甲乙 | 两个对手都在身边才够得着,否则默认射程 1 只打得到一个,"集火"在结构上不可能 |

所以引擎把两者拆开:阵营按**下标**排成 `[蓝,红,红,蓝]`(下标顺序 = 回合顺序),
而**圆桌**另排成 `0-1-3-2`(见 `GameMode.ring`)。于是 0、3 号位一队,
每个人的两个邻座都是对手、队友坐在对面 —— 裸装状态下**用【杀】打不到自己队友**。

只有 `Game.distance()` 读圆桌;下家、濒死求桃顺序、AOE 结算顺序一律仍按下标走,
那正是回合顺序。这么改之后平衡分毫未动(2000 局 48.9% / 51.1%)。

规则 AI 自己打自己 2000 局:

```
$ npm run sim 2000 4 --mode=team2v2
跑完 2000/2000 局(4 人 2v2 对抗),平均 26.4 回合
...
先后手基本持平,不需要像 1v1 那样额外补起始手牌。

怎么开:

- 网页设置页顶上的「模式」下拉切到 *2v2 对抗*
- 终端向导 `npm start -- --cli`,第二问选 *2v2 组队对抗*
- 批量跑 `npm run sim 500 4 --mode=team2v2`
- 让 Claude Code 下场:`new_game` 传 `{"mode":"team2v2","seat":0,"humanSeat":3}`
  —— 0 和 3 号位同队,于是你和 Claude 是队友(真人那边 `npm run join` 接进来)

主公技(护驾/激将/救援)在这个模式里不生效 —— 没有主公身份,和官方 2v2 一致。
...
src/
  core/
    ...
    setup.ts      开局:身份分配、发牌
    mode.ts       ★ 对局模式:身份怎么分、谁先手、怎么算赢(身份局 / 2v2)
  content/
...
先手优势在 1v1 里非常大。规则 AI 互打的实测(每档 3000 局,牌堆按官方 104 张校正后重测):
...
**一张起始牌大约值 5~8 个百分点**,+1 几乎正好落在平衡点上,所以设为默认。

也试过"先手首回合不摸牌"这个杠杆,但它一下值 18 个百分点,粒度太粗,直接矫枉过正了。

> 这组数字在**修正牌堆之后**重测过一次。旧牌堆混进了两张军争篇的牌、还有 7 张牌花色点数放错位置,量出来的是 61.0% / 53.1% / 45.4%。**改了 `DECK_TABLE` 就得重扫这张表。**
