...
**多素材转化走两步。**丈八蛇矛把任意两张手牌当【杀】,4 张手牌就是 C(4,2)=6 个选项 ——
铺成 6 个按钮既占地方又难分辨。界面把它收成三国杀该有的操作:**点装备区的【丈八蛇矛】→
点两张手牌 → 凑齐自动对上**。靠的是选项上带的 `via` 字段(这张虚拟牌是哪件装备转化的)。

界面上仍然点不到的选项(纯文字选项、结束回合)会**自动补成按钮** —— 保证任何一个合法选项
都够得着,不会出现"想选但点不到"。

顺带一提,这 6 个组合的**标签以前是一模一样的**(全是"杀(丈八蛇矛)")。人选不了是小事,
**模型也在盲选**才是大事 —— 它可能把桃和闪拆掉留下两张废牌。现在标签写清素材:
`杀(丈八蛇矛:桃[♥8]+闪[♦9])`,有测试断言 6 个组合必须有 6 个不同的标签。
